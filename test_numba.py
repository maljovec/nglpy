...
parser.add_argument('-s', dest='steps', type=int, default=1000,
...
start = time.time()
knnAlgorithm = sklearn.neighbors.NearestNeighbors(1)
knnAlgorithm.fit(X[:2])
small_edge_list = knnAlgorithm.kneighbors(X, return_distance=False)
prune_discrete(X[:2], small_edge_list, beta=1., lp=2., steps=49)
prune(X[:2], small_edge_list, beta=1., lp=2.)
end = time.time()
